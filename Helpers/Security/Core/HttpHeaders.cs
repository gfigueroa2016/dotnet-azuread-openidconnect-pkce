	...
	public class HttpHeaders : IHttpModule
	{
		...
		private void Context_PreSendRequestHeaders(object sender, EventArgs e)
		{
			...
			HttpContext.Current.Response.Headers.Add("Permissions-Policy",
				"geolocation=(), midi=(), camera=(),usb=(), magnetometer=(), sync-xhr=(), microphone=(), camera=(), gyroscope=(), payment=()");
#if DEBUG
			// SECURE: Enable Content security policy with reporting
			HttpContext.Current.Response.Headers.Add("Content-Security-Policy", $"default-src 'self'; style-src 'self' 'unsafe-inline' https:; img-src 'self' https: data:; font-src 'self' https:; script-src 'self' 'unsafe-eval' https: data: 'nonce-{CSPScriptNoncePolicy.GetScriptNonce()}'; connect-src 'self' https:; frame-ancestors 'self'; form-action 'self'; base-uri 'self'; object-src 'none'; report-uri /Security/CspReporting");
#else
			HttpContext.Current.Response.Headers.Add("Content-Security-Policy", $"default-src https:; style-src 'unsafe-inline' https:; img-src https: data:; font-src https: data:; script-src 'unsafe-eval' https: data: 'nonce-{CSPScriptNoncePolicy.GetScriptNonce()}'; connect-src https:; frame-ancestors https:; form-action https:; base-uri https:; object-src 'none'; report-uri /Security/CspReporting");
#endif
